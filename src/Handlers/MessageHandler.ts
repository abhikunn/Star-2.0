    ...
    handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
        if (!(M.chat === 'dm') && M.WAMessage.key.fromMe && M.WAMessage.status.toString() === '2') {
            ...
            */
            ...
            M.sender.username = this.client.user.name || this.client.user.vname || this.client.user.short || '❤♡ R È M ♡❤'
        ...
        if (!command) return void M.reply('Uff you fool! use commands that are in help list.')
